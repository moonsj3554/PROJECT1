...
def load_data():
    ...
    return df.reset_index(), list(map(int, year_cols))  # 연도는 숫자형으로 반환

df, available_years = load_data()

# 연도 선택 (정수형 연도 리스트를 정렬해서 사용)
selected_year = st.selectbox("연도를 선택하세요", sorted(available_years))

# 해당 연도의 상위 10개 국가 추출
selected_year_str = str(selected_year)
top10 = df[["Country", selected_year_str]].dropna().sort_values(by=selected_year_str, ascending=False).head(10)
...
fig = px.bar(
    ...
    x="Country",
    y=selected_year_str,
    text=selected_year_str,
    title=f"{selected_year}년 CO₂ 배출량 상위 10개 국가",
    labels={selected_year_str: "CO₂ 배출량 (톤)", "Country": "국가"},
    color="Country"
)
