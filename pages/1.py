...
# ===== 간단한 대륙 매핑 (수동) =====
continent_dict = {
    "China": "Asia", "India": "Asia", "Japan": "Asia", "Korea, Rep.": "Asia", "Indonesia": "Asia",
    "Saudi Arabia": "Asia", "Iran, Islamic Rep.": "Asia", "United States": "North America",
    "Canada": "North America", "Mexico": "North America", "Brazil": "South America",
    "Argentina": "South America", "Germany": "Europe", "United Kingdom": "Europe", "France": "Europe",
    "Italy": "Europe", "Russian Federation": "Europe", "South Africa": "Africa", "Nigeria": "Africa",
    "Egypt, Arab Rep.": "Africa", "Australia": "Oceania", "Qatar": "Asia",
    "United Arab Emirates": "Asia", "Turkey": "Europe"
}
...
with col_left:
    ...
    if selected_continent == "All":
        data_filtered = df[["Country Name", "Continent", "Country Code", year_col]].dropna(subset=[year_col])
    else:
        data_filtered = df[df["Continent"] == selected_continent][["Country Name", "Continent", "Country Code", year_col]].dropna(subset=[year_col])
    ...
    fig = px.line(
        ...
        y=year_col,
        markers=True,
        title=f"📈 {selected_year}년 {selected_continent if selected_continent != 'All' else '세계'} CO₂ 배출량 Top 10 (선형 그래프)",
    ...
    # ===== 지도 시각화 =====
    st.markdown("### 🗺️ Top 10 국가별 CO₂ 배출량 (지도 시각화)")
    map_fig = px.scatter_geo(
        ...
        title=f"🗺️ {selected_year}년 CO₂ 배출량 상위 10개국 (지도)",
...
with col_right:
    ...
    - **상위 10개국의 배출량**을 표, 선형 그래프, 세계 지도 3가지로 제공합니다.
    - 지도에서 각 원의 크기는 배출량 규모를 나타냅니다.
