...
with col2:
    ...
    st.markdown(f"""
- 본 그래프는 **{selected_year}년** 기준 CO₂ 배출 상위 10개국을 보여줍니다.  
- **카타르(Qatar)**의 경우, **1962→1963년 급증**은 다음과 같은 이유입니다:
    - OPEC 가입 (1961년)
    - 본격적인 석유 수출 개시 및 인프라 확장
    - 산업용 연료 소비 증가
    - 인구 대비 높은 전력 수요 및 화석연료 의존
...
with col_info:
    ...
    st.markdown(f"""
- 지도 색상은 **{selected_year}년** CO₂ 배출량 기준으로 시각화됩니다.  
- 진한 색일수록 배출량이 많고, 마우스를 올리면 배출량 수치가 표시됩니다.  
- 일부 국가는 코드 인식 누락으로 표시되지 않을 수 있습니다.  
